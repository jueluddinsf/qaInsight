...
```sh
docker run -p 3000:3000 -v /path/on/host:/app/data voya/playwright-reports-server:latest
```
...
docker run -p 3000:3000 \
  ...
  voya/playwright-reports-server:latest
```
...
curl --location --request PATCH 'localhost:3000/api/config' \
  ...
  --form 'headerLinks="{\"someLink\": \"https://example.com\", \"wiki\": \"https://wiki.voya.com/\"}"'
```
